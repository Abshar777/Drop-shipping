...
export default function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError("");
    if (!form.email.trim()) return setError(t.auth.errors.email_required);
    if (!form.password) return setError(t.auth.errors.password_required);
    ...
    try {
      ...
      let data: { error?: string; code?: AuthErrorCode } = {};
      try {
        data = await res.json();
      } catch {
        // Non-JSON response: handled as a server error below.
      }
      if (res.ok) {
        router.push("/");
        router.refresh();
      ...
      setError((data.code && t.auth.errors[data.code]) || (res.status < 500 && data.error) || t.auth.errors.server_error);
    } catch {
